...
def are_credentials_configured():
    """Checks if the Dhan API credentials are set in config.ini."""
    config = configparser.ConfigParser()
    config.read('config.ini')
    client_id = config.get('dhan', 'client_id', fallback='YOUR_CLIENT_ID')
    access_token = config.get('dhan', 'access_token', fallback='YOUR_ACCESS_TOKEN')
    return client_id != 'YOUR_CLIENT_ID' and access_token != 'YOUR_ACCESS_TOKEN'
...
def get_pnl_data():
    if not are_credentials_configured():
        return jsonify({"error": "credentials_not_configured"}), 400
    try:
        df = load_pnl_data()
